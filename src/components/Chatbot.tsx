...
LƯU Ý QUAN TRỌNG: Tuyệt đối không được giải thích hay nhắc gì về đoạn mã ||LEAD_DATA|| này với khách hàng, cứ âm thầm chèn vào cuối là được.`;
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const allMessages: Message[] = [...currentMessages, assistantMessage];
      const cleanResponse = processAIResponse(aiResponse, allMessages);
      
      const cleanAssistantMessage: Message = { role: 'assistant', content: cleanResponse };
      setMessages([...currentMessages, cleanAssistantMessage]);
    } catch (error) {
